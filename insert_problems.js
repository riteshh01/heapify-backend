  ...
  try {
    ...
    for (const [patternName, problems] of Object.entries(data)) {
      ...
      for (const title of problems) {
        ...
        while (true) {
          const res = await pool.query('SELECT id FROM dsa_problems WHERE slug = $1', [slug]);
          if (res.rowCount === 0) {
            break;
          }
        ...
        await pool.query(
          `INSERT INTO dsa_problems (pattern_id, title, problem_link, slug) 
           VALUES ($1, $2, $3, $4)`,
          [patternId, title, problemLink, slug]
        );
        insertedCount++;
      }
    }
    console.log(`Successfully inserted ${insertedCount} problems.`);
  } catch (err) {
